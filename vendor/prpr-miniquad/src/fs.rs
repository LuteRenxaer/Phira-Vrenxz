...
fn load_file_android<F: Fn(Response)>(path: &str, on_loaded: F) {
    fn load_file_sync(path: &str) -> Response {
        ...
        if data.content.is_null() == false {
            ...
            // Fallback: built-in resource archives (sfx/rank/respack/Level/Tutorial...) are
            // unpacked into <getFilesDir()>/assets on first run and the process cwd is set to
            // getFilesDir(), so try the writable assets dir when the APK has no such asset.
            std::fs::read(std::path::Path::new("assets").join(path))
                .map_err(|_| Error::AndroidAssetLoadingError)
        }
    }
