...
extern "C" {
    fn rs4_spine_malloc(__size: size_t) -> *mut c_void;
    fn rs4_spine_memcpy(__dest: *mut c_void, __src: *const c_void, __n: size_t) -> *mut c_void;
}
    ...
    extension.read_file_cb.as_ref().map_or_else(
        || {
            ...
            read(str).map_or(std::ptr::null_mut(), |data| {
                let c_data = unsafe { rs4_spine_malloc(data.len() as size_t) };
                unsafe {
                    rs4_spine_memcpy(c_data, data.as_ptr().cast::<c_void>(), data.len() as size_t);
        ...
        |cb| {
            cb(unsafe { CStr::from_ptr(c_path).to_str().unwrap() }).map_or(
                ...
                |data| unsafe {
                    *c_length = data.len() as c_int;
                    let c_data = rs4_spine_malloc(data.len() as size_t);
                    rs4_spine_memcpy(c_data, data.as_ptr().cast::<c_void>(), data.len() as size_t);
                    c_data.cast::<c_char>()
                },
            )
