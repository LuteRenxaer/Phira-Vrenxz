    ...
    fn render(&mut self, ui: &mut Ui, s: &mut SharedState) -> Result<()> {
        ...
        s.render_fader(ui, |ui| {
            ...
            self.btns_scroll.render(ui, |ui| {
                let w = r.w - pad * 2.;
                let mut h = 0.;
                let item_r = Rect::new(pad, 0., r.w - pad * 2., 0.1);
                for (index, item) in self.items.iter_mut().enumerate() {
                    let is_selected = index == self.index;
                    let card_r = item_r.feather(-0.005);
                    ui.fill_path(&card_r.rounded(0.005), if is_selected { semi_black(0.2) } else { semi_black(0.05) });
                    item.btn.render_text(ui, item_r, t, &item.name, 0.7, is_selected);
                    ui.dy(item_r.h + pad);
                    h += item_r.h + pad;
                }
                let import_r = Rect::new(pad, 0., r.w - pad * 2., 0.1);
                let card_r = import_r.feather(-0.005);
                ui.fill_path(&card_r.rounded(0.005), semi_black(0.08));
                self.import_btn.render_text(ui, import_r, t, "+", 0.8, false);
                ui.dy(import_r.h + pad);
                h += import_r.h + pad;
                (w, h)
            });
        });
        ...
        s.render_fader(ui, |ui| {
            ui.fill_path(&cr.rounded(0.01), semi_black(0.3));
            let item = &self.items[self.index];
            if let Some(pack) = &item.loaded {
                ...
                let line_y = r.y + 0.02;
                ui.fill_rect(Rect::new(cr.x + 0.05, line_y, cr.w - 0.1, 0.004), semi_white(0.2));
