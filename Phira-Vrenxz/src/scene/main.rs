...
impl MainScene {
    ...
    async fn init() -> Result<()> {
        ...
        // 并行加载所有资源文件，减少加载时间
        let (sfx_large_data, sfx_data, switch_data, play_data, back_data, suspend_data, message_data, ach_data, ach_challenge_data, background_tex, icon_back_tex) = join!(
            load_file("sfx/button_large.ogg"),
            load_file("sfx/button.ogg"),
            load_file("sfx/switch.ogg"),
            load_file("sfx/play.wav"),
            load_file("sfx/back.wav"),
            load_file("sfx/suspend.wav"),
            load_file("sfx/message.wav"),
            load_file("sfx/ordinary_achievements.ogg"),
            load_file("sfx/challenge_achievements.ogg"),
            async move {
                    ...
                    match std::fs::read(custom_path) {
                        Ok(data) => match image::load_from_memory(&data) {
                            ...
                            Err(err) => {
                                ...
                                let bg = if get_data().config.old_home { "backgrounds/background_old.jpg" } else { "backgrounds/background.jpg" };
        ...
        create_sfx!(UI_ACHIEVEMENT_SOUND, ach_data);
        create_sfx!(UI_ACHIEVEMENT_CHALLENGE_SOUND, ach_challenge_data);
