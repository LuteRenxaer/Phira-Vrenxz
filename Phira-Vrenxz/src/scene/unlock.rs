...
impl UnlockScene {
    ...
    pub async fn new(
        ...
        let background = match async {
            ...
            Ok(bg) => bg,
            Err(err) => {
                warn!("Failed to load unlock background: {:?}", err);
                BLACK_TEXTURE.clone()
            }
        };
        let loading_scene = Box::new(LoadingScene::new(mode, info, config, fs, player, None, upload_fn, update_fn, save_fn, None).await?);
    ...
    fn render(&mut self, tm: &mut TimeManager, ui: &mut prpr::ui::Ui) -> Result<()> {
        ...
        match self.state {
            ...
            State::Transforming => {
                ...
                if t < 0.5 {
                    let pad = 0.07;
                    let alpha = if t < 0.5 { 1. - t as f32 / 0.5 } else { 0. };
                    ui.loading(
                        1. - pad,
                        top - pad,
                        t as f32,
