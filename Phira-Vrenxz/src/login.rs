...
pub struct Login {
    ...
    /// 当前处于“内联编辑”的输入框（None = 没有正在编辑，软键盘关闭）
...
/// 登录/注册页的字段，支持“点字段直接内联输入”（不弹 request_input 弹窗）。
...
impl Login {
    ...
    #[inline]
    fn field_text(&self, f: LoginField) -> &str {
        match f {
            LoginField::Email => &self.t_email,
            LoginField::Pwd => &self.t_pwd,
            LoginField::RegEmail => &self.t_reg_email,
            LoginField::RegName => &self.t_reg_name,
            LoginField::RegPwd => &self.t_reg_pwd,
        }
    }

    #[inline]
    fn field_text_mut(&mut self, f: LoginField) -> &mut String {
        match f {
            LoginField::Email => &mut self.t_email,
            LoginField::Pwd => &mut self.t_pwd,
            LoginField::RegEmail => &mut self.t_reg_email,
            LoginField::RegName => &mut self.t_reg_name,
            LoginField::RegPwd => &mut self.t_reg_pwd,
        }
    }

    /// 点字段时激活内联编辑并弹出软键盘（不弹 request_input 弹窗）。
    fn activate_field(&mut self, f: LoginField) {
        self.active = Some(f);
    ...
    /// 内联编辑逐帧处理引擎键盘输入：整批字符（中文输入法一次上屏多个字）先反转再插入
    /// 以纠正顺序，退格删除最后一个字符，回车/换行结束编辑。
    fn update_inline_keyboard(&mut self) {
        ...
        if !batch.is_empty() {
            ...
            let text = self.field_text_mut(f);
            for c in batch.chars().rev() {
                text.push(c);
            }
        }
        if is_key_pressed(KeyCode::Backspace) {
            self.field_text_mut(f).pop();
        }
    }

    /// 渲染文本：当前激活的字段追加光标 `|`；密码字段用 `*` 掩码。
    fn field_display(&self, f: LoginField, masked: bool) -> String {
        let active = self.active == Some(f);
        let s = self.field_text(f);
        if masked {
            let stars: String = "*".repeat(s.chars().count());
            if active {
                format!("{stars}|")
            } else {
                stars
    ...
    pub fn touch(&mut self, touch: &Touch, t: f32) -> bool {
        ...
        if self.show {
            ...
            if self.input_email.touch(touch, t) {
                self.activate_field(LoginField::Email);
                return true;
            }
            if self.input_pwd.touch(touch, t) {
                self.activate_field(LoginField::Pwd);
                return true;
            }
            if self.input_reg_email.touch(touch, t) {
                self.activate_field(LoginField::RegEmail);
                return true;
            }
            if self.input_reg_name.touch(touch, t) {
                self.activate_field(LoginField::RegName);
                return true;
            }
            if self.input_reg_pwd.touch(touch, t) {
                self.activate_field(LoginField::RegPwd);
                return true;
            }
    ...
    pub fn render(&mut self, ui: &mut Ui, t: f32) {
        ...
        if self.show || self.fader.transiting() {
            ...
            // 预先把字段显示文本（含光标）算好，避免在嵌套闭包里借用整个 self
            let d_email = self.field_display(LoginField::Email, false);
            let d_pwd = self.field_display(LoginField::Pwd, true);
            ...
            self.fader.for_sub(|f| {
                f.render(ui, t, |ui| {
                    ...
                    ui.scissor(wr, |ui| {
                        ...
                        self.input_reg_email.render_input(ui, r, t, &d_reg_email, tl!("email"), 0.62);
                        r.y += r.h + 0.02;
                        self.input_reg_name.render_input(ui, r, t, &d_reg_name, tl!("username"), 0.62);
                        r.y += r.h + 0.02;
                        self.input_reg_pwd
                            .render_input(ui, r, t, &d_reg_pwd, tl!("password"), 0.62);
                        let h = 0.09;
                        let pad = 0.05;
                        ...
                        self.input_email.render_input(ui, r, t, &d_email, tl!("email"), 0.62);
                        r.y += r.h + 0.04;
                        self.input_pwd.render_input(ui, r, t, &d_pwd, tl!("password"), 0.62);
