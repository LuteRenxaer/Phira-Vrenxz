...
pub struct LibraryPage {
    ...
    // 内置谱修改弹窗
    modify_btn: DRectButton,
    show_modify_dialog: bool,
    modify_aspect: String,
    modify_force_aspect: bool,
    modify_new_speed: bool,
    modify_attach_ui: bool,
    modify_close_btn: DRectButton,
    modify_aspect_btn: DRectButton,
    modify_force_btn: DRectButton,
    modify_speed_btn: DRectButton,
    modify_attach_btn: DRectButton,
...
impl LibraryPage {
    pub fn new(icons: Arc<Icons>, rank_icons: [SafeTexture; 8]) -> Result<Self> {
        ...
        xcsim_server_btn: DRectButton::new(),
            ...
            modify_btn: DRectButton::new(),
            show_modify_dialog: false,
            modify_aspect: "1.77778".to_string(),
            modify_force_aspect: false,
            modify_new_speed: false,
            modify_attach_ui: false,
            modify_close_btn: DRectButton::new(),
            modify_aspect_btn: DRectButton::new(),
            modify_force_btn: DRectButton::new(),
            modify_speed_btn: DRectButton::new(),
            modify_attach_btn: DRectButton::new(),
...
impl LibraryPage {
    /// 进入内联搜索（不再弹 request_input 弹窗），弹出软键盘。
    ...
    /// 内联搜索：整批字符（中文输入法一次上屏多个字）先反转再插入以纠正顺序，
    /// 回车结束并返回 true。
    fn poll_search_inline(&mut self) -> bool {
        if !self.search_active {
            return false;
        }
        ...
        if !batch.is_empty() {
            // 与旧 InputDialog 一致：输入法一次提交的一批字符按反序到达，反转后才是正确顺序
    ...
    fn render_modify_toggle(ui: &mut Ui, x: f32, y: f32, label: &str, on: bool, btn: &mut DRectButton, t: f32) {
        ui.text(label)
            .pos(x, y)
            .anchor(0., 0.)
            .size(0.4)
        ...
        let sw = 0.12;
        let sh = 0.06;
        let sr = Rect::new(x + 0.5, y - 0.005, sw, sh);
        btn.render_shadow(ui, sr, t, |ui, _| {
            ui.fill_path(&sr.rounded(sh / 2.), if on { Color::new(0.3, 0.7, 0.3, 1.) } else { semi_black(0.5) });
            let cr = if on {
                Rect::new(sr.right() - sh + 0.005, sr.y + 0.005, sh - 0.01, sh - 0.01)
            } else {
                Rect::new(sr.x + 0.005, sr.y + 0.005, sh - 0.01, sh - 0.01)
            };
            ui.fill_path(&cr.rounded(cr.h / 2.), WHITE);
        });
    }
...
impl Page for LibraryPage {
    ...
    fn touch(&mut self, touch: &Touch, s: &mut SharedState) -> Result<bool> {
        ...
        if !choose_cover {
            ...
            if self.show_modify_dialog {
                let dr = Ui::dialog_rect().nonuniform_feather(0.04, 0.05);
                // 点击弹窗外关闭
                if !dr.contains(touch.position) {
                    self.show_modify_dialog = false;
                    return Ok(true);
                }
                if self.modify_close_btn.touch(touch, t) {
                    self.show_modify_dialog = false;
                    return Ok(true);
                }
                if self.modify_aspect_btn.touch(touch, t) {
                    self.finish_search();
                    request_input("modify_aspect", InputBox::new().default_text(&self.modify_aspect));
                    return Ok(true);
                }
                if self.modify_force_btn.touch(touch, t) {
                    self.modify_force_aspect = !self.modify_force_aspect;
                    return Ok(true);
                }
                if self.modify_speed_btn.touch(touch, t) {
                    self.modify_new_speed = !self.modify_new_speed;
                    return Ok(true);
                }
                if self.modify_attach_btn.touch(touch, t) {
                    self.modify_attach_ui = !self.modify_attach_ui;
                    return Ok(true);
                }
                return Ok(true);
            }
        ...
        if !matches!(self.tabs.selected().ty, ChartListType::Local | ChartListType::Builtin) {
            // 加载中禁止切换分页
            let is_loading = self.xcsim_task.is_some() || self.online_task.is_some();
            if !is_loading {
        ...
        match self.tabs.selected().ty {
            ...
            ChartListType::Builtin => {
                ...
                if self.modify_btn.touch(touch, t) {
                    button_hit();
    ...
    fn update(&mut self, s: &mut SharedState) -> Result<()> {
        ...
        if self.poll_search_inline() {
            // 回车结束内联搜索后，和原来 request_input 确认后一样刷新列表
            if is_local {
                self.sync_local(s);
            } else if is_xcsim {
                self.current_page = 0;
        ...
        if let Some((id, text)) = take_input() {
            ...
            } else if id == "modify_aspect" {
                self.modify_aspect = text;
            ...
            match self.multi_operation_options[self.multi_operation_menu.selected()] {
                ...
                "multi-export" => {
                    ...
                    if !non_existent.is_empty() {
                        ...
                        request_export(format!("Phira-Vrenxz-export-{}.zip", chrono::Local::now().format("%Y%m%d-%H%M%S")));
                    }
                }
                "multi-create-fav" => {
                    self.multi_operation_menu.dismiss(t);
    ...
    fn render(&mut self, ui: &mut Ui, s: &mut SharedState) -> Result<()> {
        ...
        s.render_fader(ui, |ui| {
            ...
            self.tabs.render(ui, rt, r, |ui, list| {
                list.view.render(ui, r.feather(-0.015), t);
                Ok(())
            })
        })?;
            ...
            s.render_fader(ui, |ui| {
                ...
                if let Some(selected) = &mut self.tabs.selected_mut().view.multi_select {
                    self.multi_operation_btn.render_shadow(ui, r, t, |ui, path| {
                        ...
                        ui.fill_rect(cr, (*self.icons.r#mod, cr, ScaleType::Fit, WHITE));
                    });
                    ...
                    self.multi_select_btn.render_shadow(ui, sr, t, |ui, path| {
                        ...
                        ui.fill_rect(ir, (*self.icons.select, ir, ScaleType::Fit, WHITE));
                        ui.text(text)
                    ...
                    self.multi_select_cancel_btn.render_shadow(ui, r, t, |ui, path| {
                        ui.fill_path(&path, semi_black(0.3));
                        let cr = r.feather(-0.01);
                        ui.fill_rect(cr, (*self.icons.close, cr, ScaleType::Fit, WHITE));
                    });
                    r.x -= r.w + 0.02;
                }
                ...
                if chosen == ChartListType::Local && !multi_select {
                    self.import_btn.render_shadow(ui, r, t, |ui, path| {
                        ui.fill_path(&path, semi_black(0.3));
                        let cr = r.feather(-0.01);
                        ui.fill_rect(cr, (*self.icons.plus, cr, ScaleType::Fit, WHITE));
                    });
                    r.x -= r.w + 0.02;
                }

                if chosen == ChartListType::Builtin {
                    self.modify_btn.render_shadow(ui, r, t, |ui, path| {
                        ui.fill_path(&path, semi_black(0.3));
                        let cr = r.feather(-0.01);
                        ui.fill_rect(cr, (*self.icons.r#mod, cr, ScaleType::Fit, WHITE));
                    });
                    r.x -= r.w + 0.02;
                } else if chosen != ChartListType::Local {
                    self.filter_btn.render_shadow(ui, r, t, |ui, path| {
                        ui.fill_path(&path, semi_black(0.3));
                        let cr = r.feather(-0.01);
                        ui.fill_rect(cr, (*self.icons.filter, cr, ScaleType::Fit, WHITE));
                    });
                    r.x -= r.w + 0.02;
                } else if !multi_select {
                    let active = self.current_fav_index.is_some();
                    self.fav_btn.render_shadow(ui, r, t, |ui, path| {
                        ui.fill_path(&path, if active { semi_black(0.5) } else { semi_black(0.3) });
                        let cr = r.feather(-0.01);
                        if active {
                            ui.fill_rect(cr, (*self.icons.star, cr, ScaleType::Fit, Color::from_rgba(255, 193, 7, 255)));
                        } else {
                            ui.fill_rect(cr, (*self.icons.star_outline, cr, ScaleType::Fit, WHITE));
                        }
                    });
                    r.x -= r.w + 0.02;
                }
                ...
                self.order_btn.render_shadow(ui, r, t, |ui, path| {
                    ui.fill_path(&path, semi_black(0.3));
                    let cr = r.feather(-0.01);
                    ui.fill_rect(cr, (*self.icons.order, cr, ScaleType::Fit, WHITE));
                });
                ...
                r.w = r.h;
                if !empty {
                    ui.fill_rect(r, (*self.icons.close, r, ScaleType::Fit, WHITE));
                    self.search_clr_btn.set(ui, r);
                    r.x += r.w;
                }
                ui.fill_rect(r, (*self.icons.search, r, ScaleType::Fit, WHITE));
                let display = if self.search_active {
        ...
        if self.show_modify_dialog {
            ...
            ui.fill_rect(sr, semi_black(0.6));
            let dr = Ui::dialog_rect().nonuniform_feather(0.04, 0.05);
            ui.fill_path(&dr.rounded(0.03), Color::new(0.12, 0.12, 0.14, 0.98));

            // 标题
            ui.text(tl!("library-modify-builtin"))
                .pos(dr.center().x, dr.y + 0.04)
                .anchor(0.5, 0.)
                .size(0.5)
                .draw_using(&prpr::core::BOLD_FONT);

            // 关闭按钮
            let close_r = Rect::new(dr.right() - 0.08, dr.y + 0.02, 0.06, 0.06);
            self.modify_close_btn.render_shadow(ui, close_r, t, |ui, _| {
            ...
            // 宽高比
            ui.text(tl!("library-aspect-ratio"))
                .pos(dr.x + 0.06, y)
                .anchor(0., 0.)
                .size(0.4)
                .draw();
            let input_r = Rect::new(dr.right() - 0.3, y - 0.01, 0.24, 0.07);
            self.modify_aspect_btn.render_shadow(ui, input_r, t, |ui, _| {
                ui.fill_path(&input_r.rounded(0.01), semi_black(0.5));
                ui.text(&self.modify_aspect)
                    .pos(input_r.center().x, input_r.center().y)
                    .anchor(0.5, 0.5)
                    .size(0.35)
                    .no_baseline()
                    .draw();
            ...
            // 强制宽高比
            Self::render_modify_toggle(ui, dr.x + 0.06, y, tl!("library-force-aspect").as_ref(), self.modify_force_aspect, &mut self.modify_force_btn, t);
            y += row_h;

            // 新速度缓动
            Self::render_modify_toggle(ui, dr.x + 0.06, y, tl!("library-new-speed-easing").as_ref(), self.modify_new_speed, &mut self.modify_speed_btn, t);
            y += row_h;

            // UI绑定修复
            Self::render_modify_toggle(ui, dr.x + 0.06, y, tl!("library-ui-attach-fix").as_ref(), self.modify_attach_ui, &mut self.modify_attach_btn, t);
        }
