label = 譜面庫
local = 本地
popular = 熱門
page = 第 { $current } / { $total } 頁
prev-page = 上一頁
next-page = 下一頁
not-opened = 敬請期待
failed-to-load-online = 載入線上譜面失敗
import = 匯入
offline-mode = 離線模式下無法載入線上譜面
must-login = 登入才可查看線上譜面
order-by = 排序方式：{ $order }
order-asc = 升序
order-desc = 降序
order-updated = 排序已更新
order-update-failed-sort = 只能在預設排序方式下更新排序
fav-synced = 收藏夾已同步
fav-sync-failed = 收藏夾同步失敗
multi-select-status = 已選中 { $count } 項
multi-export = 打包多個譜面
multi-create-fav = 加入到新收藏夾
multi-manage-fav = 管理收藏
multi-delete = 刪除
multi-delete-confirm = 確定要刪除這 { $count } 個譜面嗎？該操作無法撤銷。
multi-deleted = 已刪除
multi-export-title = 選擇匯出位置
multi-export-no-file = 請在匯出前下載所有選中的譜面。未下載：{ $charts }
multi-exporting = 匯出中 ({ $current } / { $total })
multi-added-to-fav = 已加入到收藏夾
multi-removed-from-fav = 已從收藏夾移除
multi-select-all = 選取全部
multi-select-invert = 反向選取
fav-created = 收藏夾建立成功
exported = 已匯出

xcsim-username-prompt = 請輸入使用者名稱/電子郵件
xcsim-api-prompt = XC-SIM API 位址
xcsim-password-prompt = 請輸入密碼
xcsim-login-success = XC-SIM 登入成功
xcsim-login-failed = XC-SIM 登入失敗
xcsim-api-updated = XC-SIM API 位址已更新
xcsim-download-updated = XC-SIM 下載位址已更新
xcsim-login = 登入
xcsim-server = 伺服器
xcsim-not-logged-in = 未登入
xcsim-logging-in = 登入中...

filter-rpe-charts = RPE 譜面 (*.pez)
filter-zip-charts = ZIP 譜面 (*.zip)
